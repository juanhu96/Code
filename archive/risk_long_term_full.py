...
SAMPLE = pd.read_csv('Data/FULL_2018_INPUT_FIRST_NOILLICIT_Y.csv', delimiter = ",")
...
SAMPLE_STUMPS = pd.read_csv('Data/FULL_2019_ALERT_STUMPS' + str(0) + '_LONGTERM.csv', delimiter = ",")
for i in range(1, N):
...
for outcome in outcome_list:
    ...
    riskslim_results_type.to_csv('../../../Result/result_longterm_single_2018first.csv')
    ...
    risk_summary_balanced = slim.risk_nested_cv_constrain(X=x,
                                                          ...
                                                          max_coef_number=6,
                                                          new_constraints=new_constraints,
                                                          c=[1e-5, 1e-4, 1e-3], # greater regularization term
                                                          class_weight = 'balanced',
                                                          seed=42)
    ...
    balanced = pd.DataFrame.from_dict(balanced, orient='index', columns=['Long term'])
    riskslim_results_type = balanced.T
    riskslim_results_type.to_csv('../../../Result/Longterm/result_fullfeature.csv')
    '''
