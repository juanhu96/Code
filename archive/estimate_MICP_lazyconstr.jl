...
function estimate_MICP_lazyconstr(N, C0, max_point, feature_case, integer_relaxation, max_runtime, tol_gap, num_threads)
    ...
    @constraint(model, [j=1:num_feature, t=1:10:num_order[j]-1, p=1:max_point], theta[j,t,p] <= theta[j,t+1,p])
    ...
    if integer_relaxation == true
        ...
        for j in 1:num_feature
            ...
            for p in 1:max_point
                for t in 1:(num_order[j]-1)
                    if JuMP.value.(theta[j,t + 1,p]) - JuMP.value.(theta[j,t,p]) > max_gap
                        t_max = t
                        p_max = p
                        max_gap = JuMP.value.(theta[j,t + 1,p]) - JuMP.value.(theta[j,t,p])
                    end
                end
            end

            push!(cutoff, x_order[j][t_max])
            push!(point, p_max)
        ...
        CSV.write("../../Result/Conic/conic_opt_" * string(N) * "_" * feature_case * "_lp.csv", df)
        ...
        df = DataFrame(selected_feature = selected_feature, cutoff = cutoff, intercept = intercept, point = point)
        CSV.write("../../Result/Conic/conic_opt_" * string(N) * "_" * feature_case * ".csv", df)
    end
