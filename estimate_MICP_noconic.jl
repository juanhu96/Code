...
function estimate_MICP_noconic(dataset, N, C0, max_point, feature_case, integer_relaxation, max_runtime, tol_gap, num_threads)
    ...
    z = df[!, :long_term_180]
    z[z .== 0] .= -1 # avoid trivial solution
    ...
    set_attribute(model, "MSK_IPAR_PRESOLVE_USE", true)

    # When relaxations in the the Branch-and-Bound tree are linear optimization problems (e.g., in MILO or when solving MICO probelms with the Outer-Approximation method)
    # it is usually best to employ the dual simplex method for their solution. In rare cases the primal simplex method may actually be the better choice, and this can be set with the parameter
    # set_attribute(model, "mioNodeOptimizer", "dualSimplex")
    ...
    cons4 = @constraint(model, [i = 1:num_obs], u[i] == sum(p * sum(theta[j, v_order[j][i], p] for j=1:num_feature) for p=1:max_point))


    obj = @objective(model, Min, sum(-z[i] * u[i] for i=1:num_obs))
    ...
    if integer_relaxation == true
        CSV.write("../../Result/Conic/conic_opt_" * string(N) * "_" * feature_case * "_noconic_lp.csv", df)
    else
        CSV.write("../../Result/Conic/conic_opt_" * string(N) * "_" * feature_case * "_noconic.csv", df)
    end
