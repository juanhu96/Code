...
def log_loss_value(Z, rho):
    ...
    loss_value[pos_idx] = np.log1p(np.exp(-stretch_constant * scores[pos_idx]))
    loss_value[~pos_idx] = -stretch_constant * scores[~pos_idx] + np.log1p(np.exp(stretch_constant * scores[~pos_idx]))
    loss_value = loss_value.mean()
...
def log_loss_value_and_slope(Z, rho):
    ...
    loss_value[~pos_idx] = -stretch_constant * scores[~pos_idx] + np.log1p(exp_scores_neg)
    loss_value = loss_value.mean()
    ...
    # print('Before log_probs and loss slops:', log_probs, loss_slope)

    exp_scores_pos = np.exp(-stretch_constant * scores[pos_idx])
    exp_scores_neg = np.exp(stretch_constant * scores[~pos_idx])
    log_probs = np.empty_like(scores)
    log_probs[pos_idx]  = 1.0 / (1.0 + exp_scores_pos)
    log_probs[~pos_idx] = exp_scores_neg / (1.0 + exp_scores_neg)
    loss_slope = Z.T.dot(log_probs - 1.0) / Z.shape[0]
    #print('After log_probs and loss slops:', log_probs, loss_slope)
    # return
...
def log_probs(Z, rho):
    ...
    pos_idx = scores > 0
    log_probs = np.empty_like(scores)
    log_probs[pos_idx]  = 1.0 / (1.0 + np.exp(-stretch_constant * scores[pos_idx]))
    log_probs[~pos_idx] = np.exp(stretch_constant * scores[~pos_idx]) / (1.0 + np.exp(stretch_constant * scores[~pos_idx]))
