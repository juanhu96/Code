...
beta = 10
arr_intercept = [0]
arr_MME = np.exp(beta * np.arange(1, 20) / 19)  # 19
arr_methadone_MME = np.exp(beta * np.arange(1, 9) / 8)  # 8
arr_prescriber = np.exp(beta * np.arange(1, 9) / 8)  # 8
arr_pharmacies = np.exp(beta * np.arange(1, 9) / 8)  # 8
arr_benzo = np.exp(beta * np.arange(1, 7) / 6) # 6
arr_days = np.exp(beta * np.arange(1, 9) / 8) # 8

scaled_arr_MME = arr_MME / np.sum(arr_MME)
scaled_arr_methadone_MME = arr_methadone_MME / np.sum(arr_methadone_MME)
scaled_arr_prescriber = arr_prescriber / np.sum(arr_prescriber)
scaled_arr_pharmacies = arr_pharmacies / np.sum(arr_pharmacies)
scaled_arr_benzo = arr_benzo / np.sum(arr_benzo)
scaled_arr_days = arr_days / np.sum(arr_days)

arr = np.concatenate((arr_intercept, scaled_arr_MME, scaled_arr_methadone_MME,\
                      scaled_arr_prescriber, scaled_arr_pharmacies, scaled_arr_benzo, scaled_arr_days))
...
def setup_objective_functions(compute_loss, L0_reg_ind, C_0_nnz):
    ...
    # get_objval = lambda rho: compute_loss(rho) + np.sum(C_0_nnz * (rho[L0_reg_ind] != 0.0)) + 1 * np.sum(arr * rho)
