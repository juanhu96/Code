...
MME = LTOUR %>% 
  ...
  mutate(Group = ifelse(Group == 1, "Yes", "No"), Feature = "Daily MME >= 40")
...
SUMMARY = rbind(PRIOR, PRESCRIBER, PHARMACY, DAYS, MME, HMFO, LONGACT)
SUMMARY$Feature = factor(SUMMARY$Feature, levels=c("Prior opioid Rx", 
                                                   ...
                                                   "Top opioid dispenser"))


ggplot(data=SUMMARY) + facet_wrap(~Feature, ncol=7) +
  ...
  geom_text(aes(x=Group, y=LTOUR+0.04, label=scales::percent(LTOUR)), size=4.2, color="black") +
...
ggplot(COUNTY_SUMMARY, aes(x = PropPos * 100, y = county)) +
  ...
  geom_text(aes(x = max(PropPos * 100, LTOUR) + 2, y=county, label=scales::percent(LTOUR)), size=3, color="black") +
  scale_color_manual("Mean predicted probability (LTOUR)", values = "black") +
