...
OUTLIER_PRESC <- FULL %>% filter(quantity >= 1000 | concurrent_MME >= 1000 | concurrent_methadone_MME >= 1000 |
                                   num_prescribers_past180 > 10 | num_pharmacies_past180 > 10 | num_prescriptions >= 100 | age >= 100 | age < 18)
...
################################################################################
### WARNING: DO NOT RUN!
### TEMP: PUT BACK PRODUCTNDC, PROPRIETARYNAME, LABELERNAME, ROUTENAME, DEASCHEDULE
### MAKE SURE TEMP & FEATURE MATCHES IN 

FULL_FEATURE <- read.csv(paste("FULL_OPIOID_", year ,"_ONE_FEATURE.csv", sep="")) # 83

columns_to_add <- FULL %>% select(PRODUCTNDC, PROPRIETARYNAME, LABELERNAME, ROUTENAME, DEASCHEDULE)
FULL_FEATURE <- bind_cols(FULL_FEATURE, columns_to_add)

# FULL 2018: 2902200
# FULL FEATURE 2018: 3044440 -> 2902200 after dropping <18
# patients_not_in_FULL <- anti_join(FULL_FEATURE, FULL, by = "patient_id")
# FULL_FEATURE <- FULL_FEATURE %>% filter(patient_birth_year <= year - 18)
# matching_patient_ids <- setequal(FULL_FEATURE$patient_id, FULL$patient_id)

TEST <- FULL[1:20,] 
TEST_FEATURE <- FULL_FEATURE[1:20,]

write.csv(FULL_FEATURE, paste("FULL_OPIOID_", year ,"_ONE_FEATURE.csv", sep=""), row.names = FALSE) # 88
FULL_FEATURE <- read.csv(paste("FULL_OPIOID_", year ,"_ONE_FEATURE.csv", sep="")) # check
...
# FULL <- FULL %>%
#   select(-PRODUCTNDC, -PROPRIETARYNAME, -LABELERNAME, -ROUTENAME, -DEASCHEDULE, 
#          PRODUCTNDC, PROPRIETARYNAME, LABELERNAME, ROUTENAME, DEASCHEDULE)
...
################################################################################
### WARNING: DO NOT RUN AGAIN!
### TEMP: PUT BACK PRODUCTNDC, PROPRIETARYNAME, LABELERNAME, ROUTENAME, DEASCHEDULE
### MAKE SURE TEMP & FEATURE MATCHES IN 

FULL_FEATURE <- read.csv(paste("FULL_OPIOID_", year, "_ATLEASTTWO_", batch ,"_FEATURE.csv", sep=""))
...
write.csv(FULL_FEATURE, paste("FULL_OPIOID_", year, "_ATLEASTTWO_", batch ,"_FEATURE.csv", sep=""), row.names = FALSE) # 88
