...
print("Start training full features ...")
start = time.time()
...
results_all = {"Accuracy": str(round(np.mean(risk_summary['holdout_test_accuracy']), 4)) + " (" + str(round(np.std(risk_summary['holdout_test_accuracy']), 4)) + ")",
...
results_all = pd.DataFrame.from_dict(results_all, orient='index', columns=['All'])

###############################################################################
########################### Feature Importance ################################
###############################################################################    

selected_features = ['concurrent_MME', 'concurrent_methadone_MME']
...
for feature in selected_features:
    ...
    results_all = pd.concat([results_all, results_feature], axis=1)

results_all = results_all.T
os.chdir('/mnt/phd/jihu/opioid')
results_all.to_csv('Result/feature_importance.csv')
