...
def fairness_in_auc(df, probs, labels):
    ...
    for gender in df["patient_gender"].unique():
        probs = df["score"][df["patient_gender"]==gender]
        labels = df["label_value"][df["patient_gender"]==gender]
        auc = roc_auc_score(labels, probs)
        rows.append({
...
def balance_positive_negative(df, probs, labels):
    ...
    for patient_gender in df["patient_gender"].unique():
        for outcome in df["label_value"][df["patient_gender"]==patient_gender].unique():
            scores = df["score"][(df["patient_gender"]==patient_gender)&(df["label_value"]==outcome)]
            rows.append({
                "Attribute": "patient_gender",
                "Attribute Value": patient_gender,              
                "Label": outcome,
                "Expected Score": scores.mean()
            })
