...
def create_intervals(data, columns, cutpoints):
    ...
    for i in range(len(columns)):
        ...
        ## for each feature, create intervals
        '''
        stumps = np.zeros([n, cutoff_length])
        for k in range(cutoff_length):
            for j in range(n):
        ...
        print('Start working on feature ' + str(feature))
        intervals_length = int((cutoff_length * (cutoff_length - 1)) / 2 + 1) # n choose 2 plus 1 (>= largest cutoff)
        intervals = np.zeros([n, intervals_length])
        
        k = 0
        # intervals between k_lower and k_upper
        for k_lower in range(cutoff_length):
            for k_upper in range(k_lower + 1, cutoff_length):
                # iterate over rows
                for j in range(n):
                    if feature_values[j] >= cutoff[k_lower] and feature_values[j] <= cutoff[k_upper]: intervals[j, k] = 1
                names.append(feature + str(cutoff[k_lower]) + '_' + str(cutoff[k_upper]))
                k += 1
                
        
        ## intervals: >= greatest cutoff
        for j in range(n):
            if feature_values[j] >= cutoff[cutoff_length-1]: intervals[j, k] = 1
        names.append(feature + '_ge_' + str(cutoff[k_upper]))
   
        ## store intervals
        final_data.append(intervals)
        final_names.append(names)
